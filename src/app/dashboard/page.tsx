  ...
  return (
    <div className="dash-root">
      ...
      <nav className="dash-nav">
        ...
        {/* Logo */}
        <a href="#" className="logo-text">&lt;/SCSE&gt;</a>
        ...
        <div className="desktop-nav-center">
          ...
          {/* Site links */}
          {siteLinks.map((l) => (
            <a key={l.label} href={l.href} className="nav-site-link">
              {l.label}
            </a>
          ))}
          ...
          {dashTabs.map((n) => (
            <button
              key={n.key}
              className={`nav-tab ${activeNav === n.key ? "nav-tab-active" : ""}`}
        ...
        <div className="nav-right">
          ...
          <button className="nav-logout-btn desktop-only" onClick={handleLogout}>LOGOUT</button>
        ...
        <div className="mobile-menu-inner">
          ...
          {user && (
            <div className="mobile-user-row">
              <PulsingDot color="var(--cyan)" />
              <span className="mobile-user-name">{user.fullName.toUpperCase()}</span>
            </div>
          )}

          {/* Section label */}
          <div className="mobile-section-label">// NAVIGATE</div>
          ...
          {/* Section label */}
          <div className="mobile-section-label" style={{ marginTop: 8 }}>// DASHBOARD</div>

          {/* Dashboard tabs */}
          {dashTabs.map((n, i) => (
            <button
              key={n.key}
              className={`mobile-tab ${activeNav === n.key ? "mobile-tab-active" : ""}`}
              style={{ animationDelay: mobileOpen ? `${(siteLinks.length + i) * 55}ms` : "0ms" }}
              onClick={() => { setActiveNav(n.key); setMobileOpen(false); }}
            >
              <span className="mobile-link-icon">{n.icon}</span>
              {n.label}
              {activeNav === n.key && <span className="mobile-active-dot" />}
            </button>
          ))}
          ...
          <button
            ...
            style={{ animationDelay: mobileOpen ? `${(siteLinks.length + dashTabs.length) * 55}ms` : "0ms" }}
      ...
      <main className="dash-main">
        ...
        <section className="hero-section">
          ...
          <p className="hero-sub">
            <span style={{ color:"var(--cyan)" }}>USER_ID:</span> {user?.userID}&nbsp;|&nbsp;
            <span style={{ color:"var(--cyan)" }}>ROLE:</span> {user?.role?.toUpperCase()}&nbsp;|&nbsp;
            <span style={{ color:"var(--cyan)" }}>STATUS:</span>{" "}
            <span style={{ color:"var(--pink)" }}>ACTIVE</span>
          </p>
          <div className="badge-row">
            {user?.isNitian  && <span className="badge badge-cyan">◈ NIT JAMSHEDPUR</span>}
            {user?.isFromCse && <span className="badge badge-pink">◆ SCSE MEMBER</span>}
            {user?.isPrime   && <span className="badge badge-yellow">★ PRIME ACCESS</span>}
          </div>
        </section>
        ...
        {(activeNav === "overview" || activeNav === "events") && (
          <section className="events-section">
            ...
            <div className="events-grid">
              {events.map((ev, i) => (
                <div key={i} className="event-card" style={{ animationDelay:`${i*80}ms` }}>
                  ...
                  <div className="event-corner tl" /><div className="event-corner br" />
                </div>
              ))}
            </div>
          </section>
        ...
        {activeNav === "registrations" && (
          <section className="profile-section">
            ...
            <div className="empty-panel">
              ...
              <button className="event-btn" style={{ marginTop:20 }} onClick={() => setActiveNav("events")}>
                BROWSE EVENTS ▶
              </button>
            </div>
          </section>
